...
namespace rc
{
    void RendererPanel::setFont(sf::Font &font)
    {
    ...
    void RendererPanel::update(sf::Vector2i mouse)
    {
        this->_closeRenderButton.update(mouse);
    }

    void RendererPanel::layout(float x, float y, float width, float height)
    {
        LayoutPen layout{x, y, 0};
    ...
    void RendererPanel::updateRender(const Render &render)
    {
        this->_currentRender = render;
    }

    bool RendererPanel::getViewportPixel(const sf::Vector2i &mouse, sf::Vector2i &pixel) const
    {
    ...
    bool RendererPanel::handleEvent(const sf::Event &event, const sf::Vector2i mouse)
    {
        return (this->_closeRenderButton.handleEvent(event, mouse));
    }

    void RendererPanel::draw(sf::RenderTarget &target, sf::RenderStates states) const
    {
        sf::Image image;
    ...
    CursorType RendererPanel::getCursor()
    {
